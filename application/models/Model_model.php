...
class Model_model extends CI_Model {
	...
	public function getModelsForDisplay($condArr = null)
	{		
		$models = $this->Model_model->getDatas($condArr);		
		$modelsData = array();
		foreach($models as $model)
		{
			$type = $this->Define_model->getMachineType($model->type);
			$row = array('Id'=>$model->Id, 'name'=> $type.' ('.$model->weight.' Lbs) : '.$model->model );
			$modelsData[] = $row;
		}
		return $modelsData;
	}

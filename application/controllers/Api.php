...
class Api extends CI_Controller {
	...
	public function braintree_payment()
	{
		...
		//must be update balance and transaction table

		if($result->success==true)		
			$this->reply(200, "", $result);	
		else
			$this->reply(400, "", $result);	
	}	
			...
			if($state->mode==1 && $state->estimate_time==0)
			{
				...
				if($job!=null && $job->need_sms==1)
				{
					...
					if($user!=null)
					{
						...
						//send sms
						$this->registerSMS($user->phone, $smsCtx);
	...
	public function kiosk_deposit() {
	   ...
	   if (substr($phone, 0, 1) != '1')
		...
		if($lang=="eng")		
		{
			$this->registerSMS($phone, "New balance is ".$user->balance);
			//$this->Sms_model->insertData(array('phone'=>$phone, 'code'=>"New balance is ".$user->balance));
						
		}
		else
		{
			$this->registerSMS($phone, "El nuevo saldo es de ".$user->balance);
	...
	public function send_sms() {
		...
		if($this->registerSMS($phone, $code))
		{
		...
		{
			return $this->reply(401, 'Registering sms failed!', null);
		}
	...
	public function verify_new_track()
	{
		$phone = $this->input->post('phone');
		if($phone=="") 
			return $this->reply(400, 'invalid phone number', null);
		...
		{
			$this->Track_model->insertData(array('phone'=>$phone, 'model_id'=>$model->Id, 'used'=>0));
			$used[]= array('model_id'=>$model->Id, 'used'=>0);
		}
		$this->reply(200, 'success', array('phone'=>$phone, 'used'=>$used));
	}
	...
	public function washer_states()	
	{
		...
		foreach($machines as $machine)
		{
			...
			if($busy)
			{
				...
				$percent = ($total - $machine->estimate_time) * 100 / $total;
				if($percent > 100)
					$percent = 100;						
			}
			$row[] = array('id'=>$machine->machine_id, 'busy'=>$busy, 
				'weight'=>intval($model->weight), 'progress'=>$percent);
		}		
		return $this->reply(200, "OK", $row);
	}
	...
	public function registerJobFromOnlinePayment()
	{
		$token = $this->input->post('token');
		$user = $this->checkToken($token);
		if($user==null)
		...
		if($paymentMethod=="")
			return $this->reply(400, 'invalid paymentMethod, params=> paymentMethod', null);
		...
		if($totalCharge > 0)
		{
			$this->CreditIncome_model->insertData(array('user_id'=>$user->Id, 
			'method'=>$paymentMethod, 'price'=>$totalCharge, 'dt'=>date("Y-m-d H:i:s")));
		}
		...
		foreach($usedstates as $usedstate)
		{
			...
			$cond = array('phone'=>$user->phone, 'model_id'=>$datas[0]);
		...
		return $this->reply(200, "successful booked", $user);
	}	


	public function registerJobFromCash()
	{
		$token = $this->input->post('token');
		$user = $this->checkToken($token);
		if($user==null)
		...
		if($totalCharge > $user->balance)
			return $this->reply(400, 'balance is insufficient', null);
	...
	public function check_track()
	{
		...
		$tracks = $this->Track_model->getDatas(array('phone'=>$user->phone));
		$used = array();
		foreach($tracks as $track)	
		{
			$used[]=array('model_id'=>$track->model_id, 'used'=>$track->used);
		}
		return $this->reply(200, "ok", $used);
	}
